...
| **Time-series CV + isotonic calibration for XGBoost, not a single train/test split** | `scripts/train_xgboost.py` selects hyperparameters via `TimeSeriesSplit` (expanding-window CV, never validates on data chronologically earlier than what it trained on) rather than a random k-fold or shuffle, since feature windows are chronologically ordered and drift over time - a random split would leak future information into earlier folds' training data. The deployed model is trained on the CV's last fold only and evaluated on its held-out split, so the reported AUCPR/Brier numbers describe the exact artifact that ships, not a "probably still this good" claim. An isotonic calibrator (`app/calibration.py`, serialized as a plain JSON piecewise-linear mapping, not a pickled sklearn object) is fit on that same held-out split and applied at score time - measured, every run, to actually cut Brier score (see `docs/metrics.md` §6 for the current numbers), not just added on faith. |
| **A rolling multi-window `velocity_count` feature, not just per-window aggregates** | `throughput_eps`/`count` describe *this* 2-5s window; they say nothing about whether an entity has been unusually busy over the last ~40-100s. `app/detectors/velocity.py`'s `RollingVelocity` (mirrored offline in `scripts/train_xgboost.py`'s `add_velocity_feature`, so training and live scoring see the same feature) adds a rolling sum of `count` over the last `velocity_window_count` windows per entity - the closest analogue available here, given this pipeline only ever sees pre-aggregated windows rather than individual events, to the multi-timeframe transaction-count features real fraud feature stores build. Feeds all three multivariate detectors (Isolation Forest and the autoencoder infer their input size from the vector automatically; XGBoost needed retraining, since `feature_names` must match between training and scoring). |
| **Real SHAP values for `top_features` when XGBoost is loaded, not just a z-score heuristic** | `app/detectors/xgboost_detector.py`'s `shap_contributions` uses XGBoost's native `pred_contribs=True` - genuine per-feature Shapley attribution, not a magnitude-only proxy. XGBoost gets the highest weight of any detector in the ensemble (it's the only one trained on real ground truth, see `ensemble.py`), so its own explanation is the most trustworthy available whenever it's loaded; the z-like heuristic (deviation from the Isolation Forest's rolling mean/std) remains the fallback before a model exists. Verified two ways: a unit test checks SHAP's defining *additivity* property (per-feature contributions + bias term reconstruct the raw prediction margin, not just "some numbers came out"), and real alerts pulled from the live stack show genuinely signed contributions (e.g. a feature *pushing toward normal*, contribution -0.96) that the old always-non-negative heuristic could never produce. |
| **`data-generator`'s WS throughput ceiling reported honestly, not chased indefinitely — then actually resolved by a different tool** | `scripts/breaking_point_test.py` found the standard end-to-end path plateaus at ~190 events/s regardless of target rate, confirmed unchanged even at 2,000/s while every container's CPU stayed low - not the Rust/Kafka/ClickHouse pipeline running out of room, but `data-generator`'s single WebSocket connection to `ingestion` hitting some transport-level ceiling. Two real fixes (fire-and-forget sends; batching events into fewer, larger WS frames) were implemented, unit-tested, and verified via re-measurement *not* to move the ceiling, then reported as such rather than assumed to have worked. `scripts/kafka_load_test.py` then bypassed the WS bridge entirely (producing straight onto `raw-events`) and found the real pipeline sustains ≥29,000 events/s with zero Kafka lag growth at 2.33% CPU - confirming the WS bridge, not the pipeline, was always the constraint. See `docs/metrics.md` §7. |
